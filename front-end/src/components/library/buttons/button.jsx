...
export function Button ({
    ...
    return (
        <button onClick={onClick} className = {`${type} ${disabled ? "disabled" : ''} ${loading ? "loading" : ''} ${className} flex justify-center items-center gap-2 group`}>
            ...
            {back && (<ArrowLeft className="w-5 h-5 transition-transform duration-500 group-hover:translate-x-[-4px]" />)}
            {children}
            {start && (<ArrowRight className="w-5 h-5 transition-transform duration-500 group-hover:translate-x-1" />)}
        </button>
    )
}
