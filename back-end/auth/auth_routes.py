...
# SỬA LỖI IMPORT: Dùng .auth để import module cùng cấp (auth.py)
...
def init_db():
    ...
    conn.execute('''
        CREATE TABLE IF NOT EXISTS users (
            ...
            -- ĐÃ LOẠI BỎ: subscription TEXT DEFAULT 'free',
...
def register():
    ...
    try:
        ...
        if not firebase_user:
            return jsonify({'error': 'Invalid Firebase token or token expired'}), 401
        ...
        if existing_user:
            conn.close()
            # Nếu user đã tồn tại, coi như login thành công
            user_data = {
            ...
            refresh_token = JWTManager.create_refresh_token(existing_user['firebase_uid'])
        ...
        conn.execute(
            ...
            (firebase_user['uid'], firebase_user.get('email', 'unknown@example.com'), 'free')
        )
        conn.commit()
...
def login():
    ...
    try:
        ...
        if not user:
            # Create new user nếu chưa có
            conn.execute(
                'INSERT INTO users (firebase_uid, email) VALUES (?, ?)',
                (firebase_user['uid'], firebase_user.get('email', 'unknown@example.com'))
            )
            conn.commit()
            user = conn.execute(
                'SELECT * FROM users WHERE firebase_uid = ?', 
